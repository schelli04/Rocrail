...
WeatherDlgGen::WeatherDlgGen( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog( parent, id, title, pos, size, style )
{
	...
	m_Color = new wxPanel( m_WeatherBook, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL );
	wxBoxSizer* bSizer7;
	bSizer7 = new wxBoxSizer( wxVERTICAL );
	
	m_ColorGrid = new wxGrid( m_Color, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHSCROLL|wxVSCROLL );
	
	// Grid
	m_ColorGrid->CreateGrid( 24, 4 );
	...
	m_ColorGrid->SetRowLabelSize( 80 );
	...
	bSizer7->Add( m_ColorGrid, 0, wxALL, 5 );
	
	m_Color->SetSizer( bSizer7 );
	m_Color->Layout();
	bSizer7->Fit( m_Color );
	m_WeatherBook->AddPage( m_Color, wxT("Color"), true );
	...
	m_ColorGrid->Connect( wxEVT_GRID_CELL_CHANGE, wxGridEventHandler( WeatherDlgGen::onColorCellChanged ), NULL, this );
...
WeatherDlgGen::~WeatherDlgGen()
{
	...
	m_ColorGrid->Disconnect( wxEVT_GRID_CELL_CHANGE, wxGridEventHandler( WeatherDlgGen::onColorCellChanged ), NULL, this );
